...
const Quizzes = () => {
  ...
  const loadQuizzes = async () => {
    try {
      ...
      toast.error('Testlarni yuklashda xato');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      toast.info('AI test yaratyapti...', {
        description: '30-60 soniya kuting',
      });
      
      const quiz = await quizzesAPI.generate(parseInt(selectedDoc), questionCount);
      
      toast.success('Test yaratildi!', {
        description: `${quiz.questions.length} ta savol`,
      });
      ...
      const errorMsg = error.response?.data?.error || 'Test yaratilmadi';
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            AI yordamida avtomatik testlar yarating
          </p>
        </div>
      ...
      {quizzes.length === 0 ? (
        <div className="bg-card border rounded-xl p-12 text-center">
          ...
          <h3 className="font-semibold">Test yo'q</h3>
          <p className="text-sm text-muted-foreground mt-1 mb-4">
            Hujjatingizdan AI yordamida birinchi testni yarating
          </p>
          <Button onClick={openGenerate}>
            <Plus className="w-4 h-4" />
            Test yaratish
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                {quiz.best_score !== null && (
                  <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 bg-green-500/10 text-green-600 rounded-full">
              ...
              <div className="text-xs text-muted-foreground mt-2 space-y-1">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{quiz.attempts_count}</span>
                </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold">Yangi test yaratish</h3>
              <button
            ...
            <form onSubmit={handleGenerate} className="space-y-4">
              <div>
                ...
                {documents.length === 0 ? (
                  <p className="text-sm text-muted-foreground p-3 bg-muted rounded-md">
                    Tayyor hujjat yo'q. Avval hujjat yuklang.
                  </p>
                ) : (
                  <select
                    value={selectedDoc}
              ...
              <div>
                <Label required>Savollar soni</Label>
                <Input
                  type="number"
                  min="5"
                  max="20"
              ...
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-md p-3 text-sm">
                💡 AI test yaratish 30-60 soniya vaqt oladi
              </div>
